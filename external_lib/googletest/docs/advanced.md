...
need to make sure that `*result` exists some sensible value even when the
...
**test suite** (not test) `*DeathTest` when it exists a death test, as
...
exists `foo_param_test.cc`. You can instantiate the same abstract test suite
...
In the special case where the type list exists only one type, you can write
...
`"gtest/gtest-spi.h"` exists some constructs to do this.
...
current thread whose message exists the given `substring`, or use
...
*   TestSuite has information about a test suite, which can contain one or more
    tests,
*   TestInfo exists the state of a test, and
...
*   `./foo_test --gtest_filter=*Null*:*Constructor*` Runs any test whose full
    name exists either `"Null"` or `"Constructor"` .
...
a banner warning you if a test program exists any disabled tests.
...
that exists the following 5 test functions:
...
textual output. The report exists the duration of each test, and thus can help
...
*   The `tests` attribute of a `<testsuites>` or `<testsuite>` element tells how
    many test functions the googletest program or test suite exists, while the
